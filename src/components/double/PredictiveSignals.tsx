...
export function PredictiveSignals() {
  ...
  >([]);
  const [cyclesMap, setCyclesMap] = useState<Record<number, Cycle[]>>({});
  ...
  // 1. Carrega dados e sincroniza via realtime + polling
  useEffect(() => {
    let alive = true;
      ...
      try {
        ...
        if (data && data.length > 0 && alive) {
          setRows((prev) => {
            ...
            incrementalEngineRef.current.processBatch(newRows);
            setCyclesMap(incrementalEngineRef.current.getAllCyclesMap());
      ...
      .on(
        ...
        (payload) => {
          ...
          setRows((prev) => {
            ...
            incrementalEngineRef.current.processBatch([newItem]);
            setCyclesMap(incrementalEngineRef.current.getAllCyclesMap());
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      for (const item of active) {
        ...
        // Janela estatística: 3 ciclos anteriores para Quebra de Padrões de Cores e Análise Q, ou 5 ciclos passados para as demais
        const hist =
          isColorBreakAnalysis || isQAnalysis ? pastValid.slice(-3) : pastValid.slice(-5);
