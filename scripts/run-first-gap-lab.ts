...
async function runLab() {
  ...
  const minuteDistribution: Record<string, number> = {
    "1-5": 0, "6-10": 0, "11-15": 0, "16-20": 0, "21-30": 0, "31-60": 0, "61-120": 0,
  };
  ...
  for (let k = 1; k <= 14; k++) {
    ...
    if (count > 0) {
      ...
      console.log(`Posição ${k.toString().padStart(2)}: Amostras=${count.toString().padStart(6)} | Exato=${exPct.toFixed(2).padStart(5)}% | ±1=${v1Pct.toFixed(2).padStart(5)}% | MAE=${mae.toFixed(2).padStart(5)} min`);
    }
  }
  ...
  for (const W of [3, 5, 10]) {
    ...
    for (const cycles of allCyclesByAnalysis.values()) {
      ...
      for (let i = W; i < cycles.length; i++) {
        ...
        if (fgMulti.length > 0) {
          ...
          const chosen2 = best2.reduce((closest, curr) => Math.abs(curr - target) < Math.abs(closest - target) ? curr : closest);
          fgTop2Tracker.add(chosen2, target);

          const best3 = fgMulti.slice(0, 3).map(p => p.m);
          const chosen3 = best3.reduce((closest, curr) => Math.abs(curr - target) < Math.abs(closest - target) ? curr : closest);
    ...
    for (const [name, t] of Object.entries(trackers)) {
      ...
      console.log(`${name.padEnd(16)} | W=${W} | N=${met.totalPredictions} | Exato=${met.exactRate.toFixed(2)}% | ±1=${met.vicinity1Rate.toFixed(2)}% | ±2=${met.vicinity2Rate.toFixed(2)}% | MAE=${met.mae.toFixed(2)}m | Med=${met.medianAe}m`);
    }

    const m2 = fgTop2Tracker.getMetrics();
    modelResultsTable.push({ model: "FIRST_GAP_TOP2", window: W, ...m2 });
    console.log(`FIRST_GAP_TOP2   | W=${W} | N=${m2.totalPredictions} | Exato=${m2.exactRate.toFixed(2)}% | ±1=${m2.vicinity1Rate.toFixed(2)}% | ±2=${m2.vicinity2Rate.toFixed(2)}% | MAE=${m2.mae.toFixed(2)}m | Med=${m2.medianAe}m`);

    const m3 = fgTop3Tracker.getMetrics();
    modelResultsTable.push({ model: "FIRST_GAP_TOP3", window: W, ...m3 });
    console.log(`FIRST_GAP_TOP3   | W=${W} | N=${m3.totalPredictions} | Exato=${m3.exactRate.toFixed(2)}% | ±1=${m3.vicinity1Rate.toFixed(2)}% | ±2=${m3.vicinity2Rate.toFixed(2)}% | MAE=${m3.mae.toFixed(2)}m | Med=${m3.medianAe}m`);
  ...
  for (const mName of ["CURRENT_GAPS", "FIRST_GAP_ONLY", "HYBRID_80_20"]) {
    ...
    console.log(`${mName.padEnd(16)} | ${blockRates.map((r, i) => `B${i+1}:${r.toFixed(1)}%`).join(" | ")} | Média=${mean.toFixed(1)}% | DP=±${std.toFixed(2)}% | Amp=${amp.toFixed(1)}%`);
  }

  // 5. DISTRIBUIÇÃO PREVISTOS VS REAIS
  console.log("\n--- DISTRIBUIÇÃO DOS MINUTOS PREVISTOS VS REAIS (W = 3) ---");
  const distTargets: Record<string, number> = { "1-5": 0, "6-10": 0, "11-15": 0, "16-20": 0, "21-30": 0, "31-60": 0, "61-120": 0 };
  const distCurrent: Record<string, number> = { "1-5": 0, "6-10": 0, "11-15": 0, "16-20": 0, "21-30": 0, "31-60": 0, "61-120": 0 };
  const distFirstGap: Record<string, number> = { "1-5": 0, "6-10": 0, "11-15": 0, "16-20": 0, "21-30": 0, "31-60": 0, "61-120": 0 };
  ...
  for (const b of Object.keys(distTargets)) {
    ...
    console.log(`${b.padEnd(7)} | ${distTargets[b].toString().padStart(6)} (${tPct.padStart(5)}%) | ${distCurrent[b].toString().padStart(6)} (${cPct.padStart(5)}%) | ${distFirstGap[b].toString().padStart(6)} (${fPct.padStart(5)}%)`);
  }
